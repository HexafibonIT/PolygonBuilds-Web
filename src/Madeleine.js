...
(function() {
  ...
  Madeleine = function(options) {
    ...
    var VIEWER_HEIGHT = 450;
    var VIEWER_WIDTH  = 350;
    ...
    Madeleine.prototype.createViewer = function() {
      if (!this.options.viewer.create) this.container.appendChild(this.__renderer.domElement);
        ...
        //this.__viewer.style.display = "none";
      ...
      var fileInputChangeHandler = function() {
        ...
        if (files.length) {
          for (i = 0; i < files.length; i++) {
            ...
            if ((files[i].type != 'stl' && files[i].type != 'STL') && (files[i].name.indexOf("stl") == -1 && files[i].name.indexOf("STL") == -1)) {
